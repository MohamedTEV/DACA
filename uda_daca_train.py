...
from utils.general import (LOGGER, check_dataset, check_file, check_git_status, check_img_size, check_requirements,
                           ...
                           one_cycle, scale_coords, print_args, xywh2xyxy, print_mutation, strip_optimizer, non_max_suppression, clip_coords_target)
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze, max_det_pct = \
        ...
        opt.resume, opt.noval, opt.nosave, opt.workers, opt.freeze, opt.max_det_pct
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if opt.image_weights:
            ...
            iw = labels_to_image_weights(dataset_s.labels, nc=nc, class_weights=cw)  # image weights
            dataset_s.indices = random.choices(range(dataset_s.n), weights=iw, k=dataset_s.n)  # rand weighted idx

        mloss = torch.zeros(3, device=device)  
        mloss_daca = torch.zeros(3, device=device)  
        if RANK != -1:
        ...
        for i, ((imgs_s, targets_s, paths_s, _), (imgs_t, _, paths_t, _)) in pbar:  # batch -------------------------------------------------------------      
            ...
            with amp.autocast(enabled=cuda):
                ...
                pred_s = model(imgs_s, pseudo=True, delta=delta)  # forward          
                pseudo_s, pred_s, var_s = pred_s

                pred_t = model(imgs_t, pseudo=True, delta=delta)  # forward
                ...
                out = output_to_target(out)  # xyxy to xc,yc,w,h
                ...
                #DACA
                imgs_concat = torch.ones_like(imgs_s)*torch.mean(imgs_s)
                if out.shape[0] > 0:
                    # get best region from target
                ...
                targets_daca[:, [2, 4]] /= w
                targets_daca[:, [3, 5]] /= h
                pred_daca = model(imgs_daca, pseudo=True)  # forward
                _, pred_daca, var_daca = pred_daca
                
                # supervised detector loss term on the labelled source samples
                loss, loss_items = compute_loss(pred_s, targets_s.to(device), var_s)

                # self-supervised consistency loss term on the mixed samples
                loss_daca, loss_items_daca = compute_loss(pred_daca, targets_daca.to(device), var_daca)
                ...
                gamma = 1
                total_loss = loss + loss_daca * gamma
            ...
            # Plot #####################################################################################################################################
            if i < 3:
                # f = save_dir / f'train_GT_{epoch}_{i}.jpg'  # filename
                # plot_images(imgs_s, targets_s, paths_s, f)
               
                f = save_dir / f'imgs_augblend_{epoch}_{i}_loss_{loss.clone().detach().cpu().numpy()}_loss_t_{loss_daca.clone().detach().cpu().numpy()}__gamma{gamma}.jpg'  # filename
                plot_images(imgs_daca, targets_daca, "", f)

                f = save_dir / f'train_t1_{epoch}_{i}.jpg'  # filename
                plot_images(imgs_t, out_original, paths_t, f)
            ...
            if RANK in {-1, 0}:
                ...
                mloss_daca = (mloss_daca * i + loss_items_daca) / (i + 1)  # update mean losses
            ...
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _ = val.run(
                        ...
                        iou_thres=0.5,  # best pycocotools results at 0.65
                        single_cls=single_cls,
                        dataloader=val_loader,
                        save_dir=save_dir,
                        save_json=is_coco,
...
def parse_opt(known=False):
    ...
    parser.add_argument('--imgsz', '--img', '--img-size', type=int, default=600, help='train, val image size (pixels)')
    parser.add_argument('--rect', action='store_true', help='rectangular training')
    parser.add_argument('--resume', nargs='?', const=True, default=False, help='resume most recent training')
    parser.add_argument('--nosave', action='store_true', help='only save final checkpoint')
    parser.add_argument('--noval', action='store_true', help='only validate final epoch')
    parser.add_argument('--noautoanchor', action='store_true', help='disable AutoAnchor')
    parser.add_argument('--noplots', action='store_false', help='save no plot files')
